...
import { fetchProduct, fetchPrice, checkStock, checkFulfillment, deductStock, restoreStock, acquireLocks, releaseLocks } from './external-api.service.js';
import { logEvent, logError } from './log.service.js';
import { getOrCreateCart, findActiveCart, updateCartProducts, updateCartAddress, deleteCart, deleteActiveCartForUser, markCartAsConverted } from '../models/cart.model.js';
...
export async function addProductToCart(c) {
  try {
    ...
    const { user_id, guest_session_id, identifier, cacheKey } = cartInfo;
    ...
    console.log('[addProductToCart] Creating/getting cart:', { user_id, guest_session_id: guest_session_id ? guest_session_id.substring(0, 8) + '...' : null });
...
export async function viewCart(c) {
  try {
    ...
    console.log('[viewCart] Looking up cart:', { user_id, guest_session_id: guest_session_id ? guest_session_id.substring(0, 8) + '...' : null });
...
export async function incrementQuantity(c) {
  try {
    ...
    const products = parseJSON(cart.products, []);
    console.log('[incrementQuantity] Cart products:', products.map(p => ({ 
    ...
    if (itemIndex < 0) {
      ...
      console.log('[incrementQuantity] Available items:', products.map(p => ({ 
...
export async function decrementQuantity(c) {
  try {
    ...
    const products = parseJSON(cart.products, []);
    const itemIndex = products.findIndex(
      p => p.product_id === product_id && String(p.size) === String(size)
    );
...
export async function getCartSummary(c) {
  try {
    ...
    console.log(`[cart-summary] START for user_id=${user_id || null}, guest_session_id=${guest_session_id ? guest_session_id.substring(0, 8) + '...' : null}`);
    ...
    for (const item of products) {
      try {
        console.log(`[cart-summary] Checking fulfillment for product_id=${item.product_id}, sku=${item.sku}, size=${item.size}, qty=${item.quantity}`);
        ...
        if (res.ok) {
          ...
          console.log(`[cart-summary] Fulfillment data for ${item.sku}: success=${fulfillmentData.success}, tier=${fulfillmentData.delivery?.highest_tier}`);
    ...
    console.log(`[cart-summary] All fulfillment checks complete. Estimates count: ${deliveryEstimates.length}, highest tier: ${highestTier}`);
    ...
    const primaryEstimate = allDeliverable && deliveryEstimates.length > 0
      ? (deliveryEstimates.find(e => e.tier === highestTier) || deliveryEstimates[0])
      : null;
    ...
    if (!allDeliverable) {
      ...
      return c.json({
        success: false,
        error: failedProducts.length === products.length 
...
export async function placeOrder(c) {
  ...
  try {
    ...
    const deliveryCost = primaryDelivery ? (mode === 'express'
      ? Number(primaryDelivery.express_delivery_cost || 0)
      : Number(primaryDelivery.standard_delivery_cost || 0)) : 0;
    ...
    const highestTier = verifiedItems.reduce((acc, it) => {
    ...
    const estimatedDeliveryDate = primaryDelivery
      ? (mode === 'express' ? primaryDelivery.estimated_delivery_date_express : primaryDelivery.estimated_delivery_date_normal)
      : null;
