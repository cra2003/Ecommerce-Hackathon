...
export async function getOrCreateCart(db, user_id, guest_session_id = null) {
  ...
  await db.prepare(`
    ...
    VALUES (?, ?, ?, '[]', 'active')
  `).bind(cart_id, user_id || null, guest_session_id || null).run();
...
export async function findActiveCart(db, user_id, guest_session_id = null) {
  if (user_id) {
    ...
    console.log('[cart-model] Looking up cart by guest_session_id:', normalizedGuestId.substring(0, 12) + '...', '(full length:', normalizedGuestId.length + ')');
    
    // Also try to see if any carts exist at all (for debugging)
    const allCarts = await db.prepare(`
      SELECT cart_id, guest_session_id, status, user_id FROM carts 
      WHERE status = 'active'
      LIMIT 10
    `).all();
    console.log('[cart-model] Debug: All active carts:', allCarts.results?.map(c => ({
      cart_id: c.cart_id,
    ...
    if (!existing) {
      ...
      console.log('[cart-model] Available guest_session_ids:', allCarts.results?.filter(c => c.guest_session_id).map(c => c.guest_session_id.substring(0, 12) + '...'));
    }
