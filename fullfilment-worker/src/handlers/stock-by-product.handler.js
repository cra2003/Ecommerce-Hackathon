  ...
  try {
    ...
    if (!results || results.length === 0) {
      const sampleCheck = await c.env.DB.prepare(`
        ...
        LIMIT 5
      `).all();
      console.log(`[stock-by-product] Sample product_ids in inventory:`, sampleCheck.results?.map(r => r.product_id));
    ...
    console.log(`[stock-by-product] Total stock for product_id "${product_id}": ${totalStock} across ${Object.keys(sizeTotals).length} sizes`);
