...
export async function stockDeductHandler(c) {
  try {
    const { warehouse_id, product_id, size, quantity } = await c.req.json();
    ...
    const result = await c.env.DB.prepare(`
      ...
      WHERE sku = ? AND stock = ?
    `).bind(newStockJson, inventory.sku, inventory.stock).run();
    ...
    console.log(`[stock-deduct] SUCCESS: Stock deducted. Warehouse=${warehouse_id}, Product=${product_id}, Size=${size}, Deducted=${quantity}, Remaining=${newStock}`);
