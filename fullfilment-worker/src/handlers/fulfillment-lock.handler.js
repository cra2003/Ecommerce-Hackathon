...
export async function fulfillmentLockHandler(c) {
  ...
  try {
    ...
    console.log(`[fulfillment-lock] START: identifier=${identifier} (user_id=${user_id || 'null'}, guest_session_id=${guest_session_id || 'null'}), allocations=${allocations?.length || 0}`);
...
export async function fulfillmentUnlockHandler(c) {
  ...
  try {
    ...
    console.log(`[fulfillment-unlock] START: identifier=${identifier} (user_id=${user_id || 'null'}, guest_session_id=${guest_session_id || 'null'}), allocations=${allocations?.length || 0}`);
