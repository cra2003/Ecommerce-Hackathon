...
export async function fulfillmentCheckHandler(c) {
  ...
  try {
    ...
    console.log(`[fulfillment-check] Matched postal range: ${matchedMapping.start_postal_code}-${matchedMapping.end_postal_code}, region: ${matchedMapping.region_name}`);
    
    const priorityWarehouses = getPriorityWarehouses(matchedMapping);
    console.log(`[fulfillment-check] Priority warehouses (${priorityWarehouses.length}): ${priorityWarehouses.map(w => w.warehouse_id).join(', ')}`);
    ...
    const skuId = inventoryResults.length > 0 && inventoryResults[0].sku 
      ? inventoryResults[0].sku 
      : null; // Will be null if SKU not available, locks will be skipped
    ...
    console.log(`[fulfillment-check] STEP 4: Allocating ${quantity} units across warehouses${skuId ? ` (checking locks for SKU: ${skuId})` : ' (locks disabled - SKU not found)'}`);
    ...
    if (remainingQuantity > 0) {
      console.log(`[fulfillment-check] ERROR: Insufficient stock. Requested ${quantity}, fulfilled ${quantity - remainingQuantity}, short ${remainingQuantity}`);
      return c.json({
        success: false,
        available: false,
    ...
    console.log(`[fulfillment-check] SUCCESS: Product available, tier=${highestTier}, express=${anyExpressAvailable}, standard_cost=₹${deliveryCost.standard}, express_cost=₹${deliveryCost.express}`);
