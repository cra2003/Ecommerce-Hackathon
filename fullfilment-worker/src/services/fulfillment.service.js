  ...
  for (let i = 0; i < priorityWarehouses.length; i++) {
    ...
    if (kv && skuId) {
      try {
        ...
        console.log(`[fulfillment-check] Warehouse ${priorityWh.warehouse_id}: Total stock=${totalStock}, Locked=${lockedQuantity}, Available=${totalStock - lockedQuantity}`);
      } catch (error) {
    ...
    if (availableStock <= 0) {
      console.log(`[fulfillment-check] Warehouse ${priorityWh.warehouse_id} (priority ${i+1}): No available stock (total=${totalStock}, locked=${lockedQuantity}, available=${availableStock})`);
      continue;
    }
    ...
    console.log(`[fulfillment-check] Warehouse ${priorityWh.warehouse_id} (priority ${i+1}): Allocating ${allocatedQuantity} units (from ${availableStock} available), tier=${tier}, express=${inventory.express_available}`);
