...
function generatePostalCodeSQL() {
  ...
  cleanupPairs.forEach(({ state, region }) => {
    ...
    sqlStatements.push(
      `DELETE FROM postal_code_warehouse_map WHERE state='${stateEscaped}' AND region_name='${regionEscaped}'`
    );
  });
...
function generateInventorySQL(products, warehouseIds) {
  ...
  for (const product of products) {
    ...
    const selectedWarehouses = warehouseIds
  ...
  const tiers = [
    {
      ...
      standard_delivery_cost: 0.00,
      express_delivery_cost: 99.00,
    ...
    },
    {
      ...
      standard_delivery_cost: 99.00,
      express_delivery_cost: 249.00,
      free_delivery_threshold: 2999.00
    }
  ];
