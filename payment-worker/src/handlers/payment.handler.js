		...
		if (!res.ok) {
			...
			if (data.name === 'UNPROCESSABLE_ENTITY' || res.status === 422) {
				...
				if (errorIssues.includes('INSTRUMENT_DECLINED')) {
					...
					return c.json({
						success: false,
						error: 'Your payment method was declined. This could be due to insufficient funds, card restrictions, or the payment method not being supported. Please try a different payment method.',
						paypal_error: 'INSTRUMENT_DECLINED',
						paypal_description: errorDescriptions.join(', '),
						sandbox_note: 'If using PayPal Sandbox, try using a different test card or PayPal test account. Some test accounts are configured to decline for testing purposes.'
					}, 422);
				}
				...
				return c.json({
					success: false,
					error: 'The payment could not be processed. This usually means the order has expired, was cancelled, or the payment method was declined. Please go back and create a new payment.',
