...
export async function updateUser(db, user_id, updates) {
	...
	await db
		.prepare(`UPDATE users SET ${set}, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?`)
		.bind(...vals, user_id)
		.run();
}

export async function updateUserLastLogin(db, user_id, ip) {
	await db.prepare('UPDATE users SET last_login_ip = ?, last_login_at = CURRENT_TIMESTAMP WHERE user_id = ?').bind(ip, user_id).run();
}
