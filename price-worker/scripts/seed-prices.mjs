...
function generateOffer(basePrice) {
  ...
  const offerValue = selectedOffer.type === 'FLAT_OFF' 
    ? randomInt(selectedOffer.valueRange.min, selectedOffer.valueRange.max)
    : randomInt(selectedOffer.valueRange.min, selectedOffer.valueRange.max);
  
  const minQuantity = selectedOffer.type === 'BULK_DISCOUNT' 
    ? randomInt(2, 4) 
    : null;
  
  const maxDiscount = selectedOffer.type === 'PERCENTAGE_OFF'
    ? Math.round(basePrice * (offerValue / 100))
    : null;
  ...
  const description = selectedOffer.desc
    .replace('{value}', offerValue)
    .replace('{min}', minQuantity || '');
